...
const CategoriesPage = () => {
  ...
  const handleQuickAdd = (e: React.MouseEvent, productName: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  // Filter categories by active filter (match by id or name)
  const filteredCategories = activeFilter
    ? allCategories.filter((cat) =>
        cat.name === activeFilter || String(allCategories.indexOf(cat) + 1) === activeFilter
      )
    : allCategories;

  // Sort products within each category
  const sortProducts = (products: typeof allCategories[0]["products"]) => {
            ...
            <div key={cat.name}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                {sortProducts(cat.products).map((product, i) => (
                  <Link to={`/products/${mockProducts.find((p) => p.name === product.name)?.id || 1}`} key={product.name}>
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
